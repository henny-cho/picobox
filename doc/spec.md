...
| --- | --- | --- |
| **PicoBox Daemon** | Go 1.24.2, `x/sys/unix` | 리눅스 노드 실행 에이전트. 커널 직접 제어 (Namespaces, cgroups v2, pivot_root) |
| **Master Backend** | Go 1.24.2, gRPC v1.64.0+, Fiber | 노드 수집, 스케줄링, 이미지 레지스트리 및 API 관제탑 |
| **Web Dashboard** | TypeScript, Next.js 15+, Tailwind CSS | 클러스터 시각화. 서버 관제실 테마 웹 |
...
- **Go Version Hopping**: Go 1.25.0 이상에서는 `golangci-lint`와 최신 gRPC 생성 코드 간의 호환성 이슈(punycode 등)가 발생할 수 있습니다. 현재 프로젝트는 가장 안정적인 **Go 1.24.2**로 고정 운영합니다.
- **gRPC Symbol Integrity**: 프로토콜 생성 코드에서 `SupportPackageIsVersion9` 에러가 발생할 경우, `google.golang.org/grpc` 버전을 최소 **v1.64.0** 이상으로 수동 업그레이드해야 정합성이 맞습니다.
