...
| :--- | :--- | :--- | :---: |
| **P-1** | **즉각적인 고립 환경** | 단 1초 만에 호스트와 완전히 격리된 실행 환경(Sandbox)을 프로비저닝 | 100% |
| **P-2** | **동적 자원 할당 제어** | 실행 중인 환경의 CPU/Memory 자원을 물리적으로 제한하여 안정성 확보 | 100% |
| **P-3** | **멀티 노드 통합 대시보드** | 분산된 수십 개의 에이전트를 하나의 화면에서 실시간으로 관측 및 제어 | 100% |
| **P-4** | **무중단 상태 모니터링** | 에이전트 손실 시 즉각적인 감지 및 컨테이너 상태 영속성 보장 | 100% |
